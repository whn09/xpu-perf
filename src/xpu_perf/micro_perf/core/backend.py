...
from xpu_perf.micro_perf.core.utils import logger, get_numa_info
...
class Backend(ABC):
    def __init__(
        ...
        backend, 
        env_file=None, 
        op_defs: pathlib.Path = None,
        vendor_ops: List[pathlib.Path] = [],
        **kwargs
    ):
        ...
        self.default_envs = {}  # 从 env.json 加载的环境变量
        ...
        # 加载并处理环境变量
        self.default_envs = self._load_envs(env_file)
    ...
    def _load_envs(self, env_file):
        ...
        if not p.exists():
            return {}
        env_config = json.loads(p.read_text())
        device_name = self.backend_info.get("device_name", "")
        merged = {}
        merged.update(env_config.get("common", {}))
        merged.update(env_config.get(device_name, {}))
        return merged
    ...
    """
    def load_all_ops(self, print_provider=False):
        ProviderRegistry.load_all_vendor_impls(self.op_defs, self.vendor_ops)
        self.op_mapping = copy.deepcopy(ProviderRegistry.OP_MAPPING)
        self.op_provider_mapping = copy.deepcopy(ProviderRegistry.OP_PROVIDER_MAPPING)
        self.provider_info = ProviderRegistry.PROVIDER_INFO.copy()

        if print_provider:
            print("")
            for provider_name in self.op_provider_mapping.keys():
                if provider_name == ProviderRegistry.BASE_PROVIDER:
                    continue
                print(f"Provider: {provider_name}")
                pt = prettytable.PrettyTable()
                pt.field_names = ["op_name", "op_cls"]
                pt.align = "l"
                for op_name, op_cls in self.op_provider_mapping[provider_name].items():
